...
class DCCRNet():
    ...
    def extract_feature_maps(self, input):
        ...
        Clears the accumulator lists first so this object can be built ONCE
        (hooks registered a single time) and reused across every training
        step, instead of being rebuilt from scratch (registering/removing
        hooks) on every single step - the hook lists otherwise just grow
        forever across the whole training run.
        """
        self.encoder = []
        self.decoder = []
        self.clstm = []
        ...
        # Pass input through the model to extract feature maps. Keep the
        # model's actual output too (as self.last_output) so callers that
        # also need the prediction (e.g. distill.py's base STFT loss) can
        # reuse it instead of running a second, redundant forward pass.
        self.last_output = self.model(input)
