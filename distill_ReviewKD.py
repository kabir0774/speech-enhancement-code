...
class KnowledgeDistillation(pl.LightningModule):
    def __init__(self, teacher, student, sftf_loss, cfg):
        ...
        # ReviewKD fusion modules built ONCE (see distill.py for the same fix
        # and the reasoning: rebuilding these every training step meant they
        # never actually trained and caused GPU memory churn/stalls).
    ...
    def training_step(self, batch, batch_idx):
        ...
        # instead of a second, redundant full forward pass (see distill.py)
        ...
        feature_maps_loss['encoder'] = hcl(student_features_encoder,teacher_encoder,'conv')
        ...
        feature_maps_loss['decoder'] = hcl(student_features_decoder,teacher_decoder,'conv')


        ############## C-LSTM REAL/IMAG loss ######################
        # HCL's plain MSE can't compare the student's 32-unit CLSTM
        # against the teacher's 128-unit CLSTM directly - they're
        # different sizes with no fusion/projection defined for this
        # layer in the ReviewKD ablation (unlike encoder/decoder, which
        # go through ABF's channel-matching fusion first). This is why
        # the paper's actual CLSKD method uses a dimension-agnostic
        # similarity loss (SPKD-style) for the CLSTM layer instead - see
        # distill.py. Left at 0.0 here to keep this ablation to what it
        # actually is: HCL-based review distillation on encoder/decoder.
...
if __name__ == "__main__":
    ...
    # TEMPORARY: allows `MAX_EPOCHS=2 python distill_ReviewKD.py` for a quick
    ...
    resume_ckpt = find_latest_checkpoint('./checkpoint_ReviewKD')
    if resume_ckpt:
    ...
    torch.save(to_save,os.path.join('./checkpoint_ReviewKD', "best_model_ReviewKD.pth"))
