...
def build_review_kd(feature_maps, ft_type):
    # in_channels == cfg.kernel_num_student, matching the student's
    # encoder/decoder feature maps as captured by feature_extraction.py
    # (post real+imag concat). See the note in DCCRNet_mini.py: the raw
    # architecture is built at HALF of cfg.kernel_num_student so that,
    # after concatenation, the feature maps come back out at exactly
    # these values - and so the student's total parameter count matches
    # the CLSKD paper's reported 0.23M instead of overshooting it.
    if ft_type == 'encoder':
